...
local function dot(ix, iy, x, y)
	local gradient = generate_gradient(ix, iy)
...
function M.noise(x, y)
	...
	local n0 = dot(x0, y0, x, y)
	local n1 = dot(x1, y0, x, y)
	local ix0 = lerp(n0, n1, sx)

	n0 = dot(x0, y1, x, y)
	n1 = dot(x1, y1, x, y)
	local ix1 = lerp(n0, n1, sx)
